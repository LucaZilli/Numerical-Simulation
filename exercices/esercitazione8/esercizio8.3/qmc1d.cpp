...
void initialize()
{
    constant=1;// Boolean variable which i added to the code: if it is true Psi=1, if it is false psi=f(mu,sigma2)
...
{
    //double k_elastic = 1;
    return val*val*val*val - 5.*val*val/2.0;
}
...
double variationalWaveFunction(double x)
{
    ...
    double m_mu = 0.790;
    double m_sigma2 = 0.375;
    return exp( - (x-m_mu)*(x-m_mu) / (2.*m_sigma2) ) + exp( - (x+m_mu)*(x+m_mu) / (2.*m_sigma2) ) ;
...
double variationalWaveFunction_second(double x)
{
    ...
    return ( add_minus + add_plus ) / (m_sigma2*m_sigma2);
}
...
void finalizePotentialEstimator()
{
	ofstream out;
    if(!PIGS)
        out.open("potential_T>0.dat");
    else
    {
        if (constant==1)
            out.open("potential_one.dat");
        else
            out.open("potential_psi.dat");
    }
	for(int i=0;i<timeslices;i++)
	{
...
void finalizeKineticEstimator()
{
	ofstream out;
    if(!PIGS)
        out.open("kinetic_T>0.dat");
    else
    {
        if (constant)
            out.open("kinetic_one.dat");
        else
            out.open("kinetic_psi.dat");
    }
	for(int i=0;i<timeslices;i++)
	{
...
void finalizeHistogram()
{
	ofstream out;
    if(!PIGS)
        out.open("probability_T>0.dat");
    else
    {
        if (constant)
            out.open("probability_one.dat");
        else
            out.open("probability_psi.dat");
    }
