...
GeneticAlgorithm::GeneticAlgorithm(Random* gen,int pop, int city,vector < vector <double> > coord){
    m_generator = gen;
    m_Npop=pop;
    m_Ncity=city;
    ...
    for( int individual=1; individual<=m_Npop; individual++ ){
        ...
        for( int city=2; city<=m_Ncity; city++ ){
            city_coin=int( m_generator->Rannyu(0, city_list.size() ) );
...
void GeneticAlgorithm::NewGeneration(){
    ...
    {//50% I crossover, 50% I select elements of the old generation
        coin = int( m_generator->Rannyu(0,100) );
        if(coin<80)
            offspring=CrossOver();
        else
            offspring=Selection();
...
vector < vector <int> > GeneticAlgorithm::Selection(){
    ...
    double p=4.0;
    int mom,dad;
    mom = int( pow( m_generator->Rannyu(), p ) * m_Npop );
    do
        dad = int( pow( m_generator->Rannyu(), p ) * m_Npop );
...
vector < vector <int> > GeneticAlgorithm::CrossOver(){
    ...
    int  length = int( m_generator->Rannyu() * m_Ncity/2 ); //I select a number between [0,15]
    int start = int( m_generator->Rannyu(1,m_Ncity-length) ); //I select a number between [1,Ncity-1]
...
void GeneticAlgorithm::Mutation(){
    int coin1 = int( m_generator->Rannyu(0,100) );
    int coin2 = int( m_generator->Rannyu(0,100) );
    int chromosome;
...
template <typename T> vector <T> GeneticAlgorithm::Permutation(const vector <T>& chromosome){
    ...
    int  length = int( m_generator->Rannyu() * m_Ncity-1 )+2; //I select a numberbetween [1,32]
    int start = int( m_generator->Rannyu(1,m_Ncity) ); //I select a number between [2,Ncity]
    vector <T> piece;
    for(int i=1 ; i < length; i++ )
    {
...
/*
int* GeneticAlgorithm::SendTheBest(){
    int* send=new int[m_Ncity];
    for(int i=1;i<=m_Ncity;i++) send[i-1]=m_TheBestChromosome[i-1];
    //for(int i=1;i<=m_Ncity;i++) cout<<send[i-1]<<" ";cout<<endl;
    return send;
}
 */

void GeneticAlgorithm::ReceiveTheBest(int* v){
    vector <int> theNewBest(m_Ncity);
    for(int i=1;i<=m_Ncity;i++) theNewBest[i-1]=v[i-1];
    vector <int> ordered_population;
    ordered_population = Sort_Indexes(m_fitness);//i sort indexes by the correspondent values (in the file funzioni.h)
    m_population[ordered_population[0]]=theNewBest;
    //cout<<endl<<"the new best";Print(m_population[ordered_population[0]]);cout<<endl;
    Fitness();
}
